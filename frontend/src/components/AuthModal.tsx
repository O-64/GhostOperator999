...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const { login, signupCandidate, signupRecruiter } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup' && !sanitizeInput(name)) {
    ...
    if (mode === 'signup') {
      if (role === 'candidate') {
        ...
        result = await signupRecruiter({
          name: cleanName,
          email: cleanEmail,
          password: cleanPassword,
          role: 'recruiter',
          company: sanitizeInput(companyOrTitle) || 'My Company',
          title: 'Recruiter',
        });
      }
    } else {
      result = await login(cleanEmail, cleanPassword, role);
    }
    ...
    if (result.success) {
      setSuccessMessage(
        mode === 'signup'
          ? `Account created as ${role.toUpperCase()}! Loading dashboard...`
          : `Successfully authenticated as ${role.toUpperCase()}! Loading dashboard...`
      );
      setTimeout(() => {
        onClose();
      ...
      setErrorMessage(result.error || (mode === 'signup' ? 'Sign up failed.' : 'Authentication failed.'));
    }
  };
